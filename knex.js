...
class KnexAdapter extends BaseKeystoneAdapter {
  ...
  async postConnect() {
    const isSetup = await this.schema().hasTable(
      Object.keys(this.listAdapters)[0]
    );
    ...
    const createResult = await pSettle(
      Object.values(this.listAdapters).map(listAdapter =>
        listAdapter.createTable()
      )
    );
    const errors = createResult
...
class KnexListAdapter extends BaseListAdapter {
  ...
  async createForeignKeys() {
    const relationshipAdapters = this.fieldAdapters.filter(
    ...
    await this._schema().table(this.key, table => {
      relationshipAdapters
        ...
        .forEach(adapter =>
          adapter.createForeignKey(table, this.parentAdapter.schemaName)
        );
    });
  ...
  async createAdjacencyTable(relationshipFa) {
    ...
    const rightListAdapter = dbAdapter.getListAdapterByKey(
      relationshipFa.refListKey
    );
      ...
      .forEach(path => {
        if (path === "AND" || path === "OR") {
          ...
          if (adapter) {
            ...
            this.getListAdapterByKey(otherList)._traverseJoins(
              where[path],
              aliases[key],
              aliases
            );
  ...
  async _buildManyWhereClause(path, where) {
    ...
    const matchingItems = await this.getListAdapterByKey(otherList)._itemsQuery(
      { where }
    );
    const matchingCount = await this._query()
      ...
      .groupBy(thisID);
    const matchingCountById = arrayToObject(
      matchingCount,
      thisID,
      x => x.count
    );
      ...
      Object.keys(where).map(async path => {
        if (nonJoinConditions.includes(path)) {
          ...
          const subClauses = flatten(
            await Promise.all(
              where[path].map(d =>
                this._traverseWhereClauses(d, alias, aliases)
              )
            )
          );
          whereClauses.push(b => {
          ...
          if (adapter) {
            ...
            whereClauses.push(
              ...(await this.getListAdapterByKey(
                adapter.refListKey
              )._traverseWhereClauses(
                where[path],
            ...
            // Many relationship
            whereClauses.push(
              await this._buildManyWhereClause(path, where[path])
            );
          }
        }
      })
  ...
  async _itemsQuery(args, { meta = false } = {}) {
    ...
    const whereClauses = await this._traverseWhereClauses(
      where,
      baseAlias,
      aliases
    );
    ...
    if (!meta) {
      ...
      if (orderBy !== undefined) {
        ...
        const sortKey =
          this.fieldAdaptersByPath[orderField].sortKey || orderField;
...
class KnexFieldAdapter extends BaseFieldAdapter {
  ...
  get defaultTo() {
    ...
    const resolve = () => {
      if (typeof defaultToSupplied === "undefined")
        return this.field.defaultValue;
      if (typeof defaultToSupplied === "function")
        return defaultToSupplied(knex);
  ...
  get isNotNullable() {
    ...
    return (this._isNotNullable = !!(typeof this.knexOptions.isNotNullable ===
    "undefined"
      ? this.field.isRequired ||
        (typeof this.defaultTo !== "undefined" && this.defaultTo !== null)
      : this.knexOptions.isNotNullable));
  }

  addToTableSchema() {
    throw `addToTableSchema() missing from the ${
      this.fieldName
    } field type (used by ${this.path})`;
  }
  ...
  inConditions(dbPath, f = identity) {
    return {
      [`${this.path}_in`]: value => b =>
        value.includes(null)
          ? b
              .whereIn(dbPath, value.filter(x => x !== null).map(f))
              .orWhereNull(dbPath)
          : b.whereIn(dbPath, value.map(f)),
      [`${this.path}_not_in`]: value => b =>
        value.includes(null)
          ? b
              .whereNotIn(dbPath, value.filter(x => x !== null).map(f))
              .whereNotNull(dbPath)
